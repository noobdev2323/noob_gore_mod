    ...
    if GetConVar("Noob_gore_mod_enable"):GetBool() then 
		if GetConVar("can_gib_only_npc_corpse"):GetBool() == false and ragdoll:GetClass() == "prop_ragdoll" then 
			timer.Simple(0, function()
				if IsValid(ragdoll) and not ragdoll.GORE_Corpse then
...
hook.Add("CreateEntityRagdoll", "Replace_shit_Ragdoll", function(owner, ragdoll)
	if GetConVar("Noob_gore_mod_enable"):GetBool() then
		...
		local npc_class = owner:GetClass()
		if owner.gib then
		...
		if not no_gib_ragdoll[npc_model] or not npc_class_black_list[npc_class] and owner:LookupBone("ValveBiped.Bip01_Pelvis") then
			ApplyCorpseEffects(ragdoll) 
		end
